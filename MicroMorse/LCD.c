...
{ 
	// This function displays a string on an LCD module, character by character, starting from a 
	// specified position. If a colon is encountered in the string, it toggles a colon flag to control 
	// whether the colon should be displayed.
  ...
  // Send the string character by character on LCD 
    while((*ptr != 0) && (position < 6)) {
        ...
        LCD_WriteChar(ptr, colonOn, position); // Display one character on LCD 
	...
	// position: An integer indicating the position (digit) on the LCD where the character should be displayed.
{
  ...
  LCD_Conv_Char_Seg(ch, colon, digit); // function call
